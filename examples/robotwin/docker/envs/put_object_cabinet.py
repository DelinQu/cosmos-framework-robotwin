...
class put_object_cabinet(Base_Task):
    ...
    def _init_eval_state(self):
        ...
        check_success() reads self.arm_tag and self.origin_z, but play_once() -- the
        expert script -- was their only assignment. Policy evaluation runs the expert
        on a different env object than the rollout (benchmark.get_tasks() vs
        benchmark.reset()), so the rollout env raised AttributeError on the first step
        of every episode.
        ...
        Timing is load-bearing for origin_z: it is the object's SETTLED resting height,
        which check_success() compares against with a 7mm threshold. Reading it before
        the settle steps would bias every success test on this task.
        """
    ...
    def play_once(self):
        arm_tag = self.arm_tag  # arm_tag/origin_z set by _init_eval_state() in setup_demo
