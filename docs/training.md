...
<details><summary><b>Action-Policy Post-Training (DROID / LIBERO / RoboTwin)</b></summary>

Robot action-policy recipes: DROID (`joint_pos` 8-D actions + proprioceptive state),
LIBERO (`frame_wise_relative` rot6d 10-D actions), and RoboTwin 2.0 (dual-arm ALOHA,
14-D absolute `joint_pos` + proprioceptive state), all from Cosmos3-Nano. They follow
the same Step 2/Step 3 flow below, with their own data staging
(DROID keep-ranges window filter, LIBERO suite selection, RoboTwin trajectory
generation), and are documented separately:
...
- [RoboTwin action policy](../examples/robotwin/README.md) —
  `examples/launch_sft_action_policy_robotwin_nano_clean_plus_random.sh` / `examples/launch_sft_action_policy_robotwin_nano_clean.sh`
